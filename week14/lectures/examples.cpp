...
static std::size_t dfs_size(const Graph &graph, std::size_t start,
...
static std::optional<int> shortest_distance(
...
int main() {
    ...
    std::cout << "component size = " << dfs_size(graph, 0, visited) << '\n';

    const std::vector<std::vector<char>> grid{{'.', '.', '#'},
                                              {'.', '.', '.'}};
    const auto distance = shortest_distance(grid, {{0, 0}}, {1, 2});
