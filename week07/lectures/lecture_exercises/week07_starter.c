...
static Ast* parse_unary(Parser* parser) {
    /*
     * TODO: implement the unary grammar:
     *
     *   unary -> ('+' | '-') unary | primary
     *
     * Unary plus returns the recursively parsed operand unchanged. Unary minus
     * creates AST_NEGATE. If allocation of that node fails, destroy the operand
     * because ast_create transfers child ownership only when it succeeds.
     */
